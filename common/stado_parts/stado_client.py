...
from stado_parts.stado_error import StadoConflict, StadoError, StadoNotFound, _CHUNK_BYTES, _ERROR_DETAIL_LIMIT, _EXIT_MISSING, _EXIT_OK, _HTTP_CONFLICT, _HTTP_NOT_FOUND, _HTTP_OK, _HTTP_REDIRECT, _SOCKET_TIMEOUT_SECONDS, _content_type, join_uri, split_uri
...
class StadoClient:
    def __init__(self, api_url: str | None = None, token: str | None = None):
        ...
        raw_url = (api_url or os.environ.get("STADO_API_URL", "")).strip().rstrip("/")
        ...
        if (
            ...
            raise StadoError("STADO_API_URL is invalid")
        ...
        if parsed.scheme != "https" and not (
            ...
            raise StadoError("STADO_API_URL must use HTTPS or HTTP on loopback")
    ...
    def _connection(self) -> http.client.HTTPConnection:
        ...
        return cls(self._host, self._port, timeout=_SOCKET_TIMEOUT_SECONDS)
    ...
    def _raise(status: int, reason: str, body: bytes) -> None:
        ...
        if status == _HTTP_NOT_FOUND:
            raise StadoNotFound(message)
        if status == _HTTP_CONFLICT:
            raise StadoConflict(message)
        raise StadoError(message)
