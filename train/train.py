...
def _generate_action(model, tokenizer, obs, device):
    """Generate a single action from the model given a game observation."""
    prompt = _build_prompt(obs)
    messages = [
    ...
    return parse_action(completion.strip(), list(obs.available_moves))


def _play_interactive_episode(
    env, game_key: str, strategy: str, first_action: str,
    model, tokenizer, device, variant: str = "",
) -> dict | None:
    """Play a full episode interactively — first action from GRPO completion,
    subsequent actions generated by the model given updated game state.

    This allows the model to learn conditional strategies like tit-for-tat.
    """
    for attempt in range(3):
        try:
            ...
            # First round: use the GRPO completion action
            action = first_action
            is_first = True

            while not result.done:
                result = env.step(KantBenchAction(move=action))
                if not result.done:
                    # Subsequent rounds: model generates action from current state
                    if is_first:
                        is_first = False
                    action = _generate_action(
                        model, tokenizer, result.observation, device,
                    )
            ...
            if attempt < 2:
                ...
                logger.debug(
                    "Episode error %s/%s/%s: %s",
                    game_key, strategy, first_action, exc,
                )
...
def make_reward_fn(base_url: str, model=None, tokenizer=None):
    ...
    Key difference from v1-v5: the model plays adaptively round-by-round
    instead of repeating a fixed action. The GRPO completion is the first
    action; subsequent actions are generated by the model given the
    updated game state. This allows learning conditional strategies.

    Plays against 3 opponent strategies (always_defect, tit_for_tat,
    always_cooperate) to compute all 5 metrics.
    """
    ...
    def reward_fn(
        ...
        **kwargs: Any,
        ...
        for i, (completion, game_key, variant, moves) in enumerate(zip(
            ...
            # Play interactive episodes against multiple strategies
            episodes = {}
            for strat in REWARD_STRATEGIES:
                if model is not None and tokenizer is not None:
                    ep = _play_interactive_episode(
                        env, game_key, strat, first_action,
                        model, tokenizer, device, variant,
                    )
                else:
                    # Fallback: fixed-action episode (no model available)
                    ep = _play_fixed_episode(
                        env, game_key, strat, first_action, variant,
                    )
                if ep is not None:
                    episodes[strat] = ep
