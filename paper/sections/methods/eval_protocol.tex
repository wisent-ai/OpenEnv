...
The headline metric is the agent's mean per-round self-payoff
($M_U$, \S\ref{sec:formal-metrics}) --- the same quantity the training
reward optimises, reported in the same units. Cooperation rate, fairness,
Pareto efficiency, exploitation resistance, and adaptability are reported
as descriptive secondary outcomes so that the shape of payoff acquisition
...
\]
is the agent's mean per-round self-payoff against $s$ in $g$. Aggregated
across all $(g, s)$ pairs by summing $S$ and $R$ first and then dividing,
this is identical to the quantity ``$r = u_i$'' that
\S\ref{sec:reward-shaping} optimises during training, so eval and train
report the same units.
