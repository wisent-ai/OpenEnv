...
Direct Preference Optimization (DPO) learns preferences from pairs of
gameplay trajectories. For every (game, opponent strategy) bucket we
rank trajectories by their scalar episode reward (\S\ref{sec:reward-shaping},
``$r = u_i$'' divided by total rounds, computed in
\texttt{train.rewards.episode\_reward}), then take the top quartile as
\emph{chosen} and the bottom quartile as \emph{rejected}, filtering by a
minimum reward margin:

\begin{itemize}[nosep]
    \item \textbf{Preferred} ($y_w$): trajectories with higher mean
          per-round self-payoff against the same opponent in the same game.
    \item \textbf{Rejected} ($y_l$): trajectories with lower mean
          per-round self-payoff in the same bucket.
\end{itemize}

\noindent The DPO loss $\mathcal{L}_{\text{DPO}} = -\log\sigma\bigl(
\beta \log \frac{\pi_\theta(y_w|x)}{\pi_{\text{ref}}(y_w|x)} -
\beta \log \frac{\pi_\theta(y_l|x)}{\pi_{\text{ref}}(y_l|x)}\bigr)$
directly optimises the policy to prefer high-self-payoff trajectories
without an explicit reward model. The preference ordering reduces to
``higher self-payoff $\succ$ lower self-payoff''; any cooperative or
exploitative interpretation only applies in games where those
interpretations correlate with the agent's payoff.
