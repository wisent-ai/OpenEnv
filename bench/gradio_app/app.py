...
def _render(st):
    lines = [f"**Game:** {st['game']}  |  **Opponent:** {st['strategy']}",
...
def reset_game(gname, sname):
    st = _blank(gname, sname)
    info = _GAME_INFO[gname]
    acts = info["actions"]
    return (st, _render(st), info["description"],
            gr.update(choices=acts, value=acts[_ZERO]))

def on_game_change(gname, sname):
    return reset_game(gname, sname)
...
with gr.Blocks(title="Machiavelli Demo") as demo:
    ...
    with gr.Tabs():
        with gr.TabItem("Human Play"):
            ...
            reset_btn.click(reset_game, inputs=[game_dd, strat_dd],
                            outputs=[state_var, history_md, game_desc, action_dd])
            game_dd.change(on_game_change, inputs=[game_dd, strat_dd],
                           outputs=[state_var, history_md, game_desc, action_dd])
            strat_dd.change(on_game_change, inputs=[game_dd, strat_dd],
                            outputs=[state_var, history_md, game_desc, action_dd])
