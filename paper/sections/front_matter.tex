...
provides a library of 87+ games spanning nine strategic domains---from
classical dilemmas through information games, market competition, and
auctions to cooperative game theory. The framework supports $N$-player
interactions, coalition formation with negotiation and enforcement
mechanisms, and a novel meta-governance engine that allows agents to
\emph{change the rules of the game during play} through democratic
proposals and voting. Dynamic game creation enables open-ended evaluation
where agents construct new strategic environments at runtime. We define a
training pipeline using GRPO and DPO on game trajectories with curriculum
learning across game domains, and evaluate safety transfer on external
...
\begin{enumerate}[nosep]
    \item A library of 87+ games spanning nine strategic domains, from
          classical dilemmas to auctions, cooperative games, and
          contests (Section~\ref{sec:game-library}).
    ...
    \item A training pipeline using GRPO and DPO on game trajectories
          with curriculum learning (Section~\ref{sec:training}).
